...
# Los primeros 5 vienen del pipeline de enriquecimiento (enriquecer_dataset_v2).
# Los 4 siguientes vienen de auto_etiquetar.py (sentimiento BERT por registro).
# Si el JSON de entrada NO pasó por auto_etiquetar.py, estos campos no existen
# y el bucle de extracción los rellena con 0.0 de forma segura (fila.get(c, 0.0)).
# score_queja_salud se sigue añadiendo dinámicamente al final (mejora ②).
FEATURES_NUMERICAS = [
    # ── Pipeline de enriquecimiento ──────────────────────────────────────────
    "polaridad_score",        # negatividad localizada (enriquecer_dataset_v2)
    "ifb_score",              # índice de fricción burocrática
    "impacto_escala",         # 0-3: ninguno → administrativo → económico → clínico
    "num_palabras",           # longitud del comentario
    "peso_reddit",            # log-score de validación comunitaria
    # ── auto_etiquetar.py — sentimiento BERT (nuevos) ────────────────────────
    # Nota: si el JSON no pasó por auto_etiquetar estos quedan en 0.0
    "prob_neg",               # P(NEG) de robertuito — señal directa de queja
    "prob_neu",               # P(NEU) — comentario informativo o neutral
    "prob_pos",               # P(POS) — descarta falsas alarmas positivas
    "sentimiento_num",        # NEG=2 NEU=1 POS=0 — ordinal para modelos lineales
...
def extraer_corpus(registros: list[dict]) -> tuple[
    ...
    for reg in registros:
        ...
        # fila.get(col, 0.0) garantiza que si el campo no existe (JSON sin
        # auto_etiquetar) se rellena con 0.0 en lugar de lanzar KeyError.
        # Esto hace que vectorizar_v3.py sea compatible con AMBOS flujos:
        #   · dataset_normalizado_*.json   (sin BERT → BERT cols = 0.0)
        #   · dataset_etiquetado_*.json    (con BERT → valores reales)
...
def escalar_features_numericas(feat_num_raw: list[dict]):
    ...
    # Detectar cuántas columnas BERT tienen valores reales (no todas cero)
    # para informar al usuario si el dataset viene de auto_etiquetar o no
    cols_bert   = ["prob_neg", "prob_neu", "prob_pos", "sentimiento_num"]
    bert_activo = any(
        any(fila.get(c, 0.0) != 0.0 for c in cols_bert)
        for fila in feat_num_raw[:20]   # revisar primeros 20 para rapidez
    )
    ...
    bert_tag = "✅ BERT probs activas" if bert_activo else "⚠️  BERT cols=0 (ejecuta auto_etiquetar.py primero)"
    print(f"  Features numéricas : {X_num.shape[1]} columnas — {bert_tag}")
    print(f"  score_queja_salud  : ✅ incluida")
...
def main():
  ...
  #
  # ⚠️  NOTA: con features BERT (prob_neg, prob_pos, sentimiento_num),
  # ComplementNB ya NO es el más óptimo porque Naive Bayes asume
  # independencia entre variables — pero prob_neg y sentimiento_num
  # están altamente correlacionados por construcción.
  # Para datasets híbridos (texto + probabilidades BERT) usa:
  #   LogisticRegression(C=1.0, class_weight='balanced')  ← recomendado
  #   LinearSVC(C=0.5, class_weight='balanced')           ← más rápido
  #   XGBClassifier()                                     ← si tienes >5k muestras
