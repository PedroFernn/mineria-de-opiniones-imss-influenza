#!/usr/bin/env python3
"""
limpiar_dataset_v4.py — Limpieza y depuración del dataset Reddit
Tema: Impacto de la ineficiencia del sector salud público MX frente a la influenza

Cambios v4 respecto a v3 (correcciones de rendimiento para archivos ≥ 200 MB):
  ✦ [FIX 1] _stream_ijson detecta automáticamente la ruta del array:
            prueba "item" (array crudo), "datos.item" (output de v3)
            y "item" dentro de cualquier clave dict de primer nivel.
            En v3 solo probaba "datos.item" y fallaba silenciosamente.
  ✦ [FIX 2] Escritura en streaming: los registros limpios se escriben
            directamente al archivo de salida uno a uno — ya no se
            acumula la lista `limpios` completa en RAM.
  ✦ [FIX 3] json.dump final reemplazado por un serializado incremental
            que no requiere tener todo el dataset en memoria al mismo tiempo.
  ✦ [FIX 4] cargar_registros usa siempre streaming (ijson) cuando el archivo
            supera UMBRAL_STREAMING_MB, sin importar si ijson está instalado
            (se instala automáticamente si falta).
  ✦ Toda la lógica de limpieza/relevancia de v3 se conserva intacta.
...
Uso: python3 limpiar_dataset_v4.py
"""
...
def clasificar_relevancia(texto_limpio: str, texto_normalizado: str) -> tuple[str, dict]:
    ...
    relevancia = "alta" if (sustantivos_activos and indicadores_activos) else "media"
...
def pedir_archivos() -> tuple[Path, Path]:
    ...
    print(f"\n{'='*66}")
    print("  LIMPIEZA DE DATASET v4 — ANÁLISIS DE SENTIMIENTOS  🏥")
    print(f"{'='*66}\n")
...
def main():
    ...
    print(f"\n{'='*66}")
    print("  EJECUTANDO PIPELINE DE LIMPIEZA v4...")
    print(f"{'='*66}")
