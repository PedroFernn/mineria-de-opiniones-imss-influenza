...
# Debe coincidir con TEST_SIZE en analizar_v4.py
TEST_SIZE_INFOGRAFIA = 0.20
...
def cargar(carpeta: Path) -> dict:
    ...
    if rep_path and rep_path.exists():
        ...
        d["auc_ic"]    = cv.get("roc_auc",  {}).get("ic95_pm", d["auc_ic"])
    ...
    # Calcular totales del corpus desde la matriz de confusión
    # (más fiable que hardcodear valores)
    cm = d["cm_ensamble"]
    cm_ok = len(cm) == 2 and len(cm[0]) == 2
    if cm_ok:
        tn_c, fp_c, fn_c, tp_c = cm[0][0], cm[0][1], cm[1][0], cm[1][1]
        n_test_cm = tn_c + fp_c + fn_c + tp_c
        if n_test_cm > 0 and not d["n_total"]:
            d["n_total"] = round(n_test_cm / TEST_SIZE_INFOGRAFIA)
            d["n_alta"]  = round((fn_c + tp_c) / TEST_SIZE_INFOGRAFIA)
            d["n_media"] = round((tn_c + fp_c) / TEST_SIZE_INFOGRAFIA)

    # Fallback si la CM también está vacía
    if not d["n_total"]:
        d["n_total"] = 2595
        d["n_alta"]  = 766
        d["n_media"] = 1829
...
def _defaults() -> dict:
    return {
        ...
        "f1_alta": 0.784, "auc": 0.940, "auc_ic": 0.011,
...
<div class="card" style="margin-bottom:14px">
  <p class="chart-desc">
    <strong>Gráficas de diagnóstico exportadas por el pipeline.</strong>
    Las <em>curvas de aprendizaje</em> muestran cómo evoluciona el F1 de entrenamiento
    y de validación a medida que se incrementa el tamaño del corpus: si ambas líneas
    convergen, el modelo está bien ajustado; si la brecha persiste, hay overfitting o
    falta de datos. Las <em>matrices de confusión</em> permiten comparar visualmente
    la distribución de errores entre el VotingEnsemble y la Regresión Logística individual,
    verificando que el ensamble reduce los Falsos Negativos respecto al clasificador base.
  </p>
</div>"""
...
.chart-desc {{
  ...
  border-left:2px solid rgba(124,106,240,.4);
  border-radius:0 6px 6px 0;
}}
.chart-desc strong {{ color:var(--text); font-weight:600; }}
.chart-desc em {{ color:var(--purple); font-style:normal; font-weight:600; }}
...
<div class="row row4" style="margin-bottom:14px">
  <div class="card">
    ...
    <p class="chart-desc">
      <strong>¿Qué mide?</strong> El F1-score pondera precisión y recall de cada clase según
      su frecuencia. Un valor de <em>{d['f1']:.3f}</em> significa que el modelo clasifica
      correctamente ~{round(d['f1']*100,1)}% de los comentarios teniendo en cuenta
      el desbalance entre clases. El intervalo de confianza ±{d['f1_ic']:.3f}
      confirma que el resultado es <strong>robusto</strong> y no producto del azar.
    </p>
  </div>
  <div class="card">
    ...
    <div class="metric">
      ...
      <span class="sub">IC 95%: ± {d['auc_ic']:.3f}</span>
      <span class="badge badge-blue">separación excelente</span>
    </div>
    <p class="chart-desc">
      <strong>¿Qué mide?</strong> El AUC-ROC cuantifica la capacidad del modelo de
      <em>separar</em> quejas sistémicas (clase alta) de comentarios leves.
      Un valor de <em>{d['auc']:.3f}</em> indica que ante dos comentarios aleatorios
      —uno de cada clase— el modelo asigna mayor probabilidad al correcto
      el {round(d['auc']*100,1)}% de las veces. Por encima de 0.90 se considera
      <strong>discriminación excelente</strong>.
    </p>
  </div>
  <div class="card">
    ...
    <p class="chart-desc">
      <strong>¿Por qué importa?</strong> La clase alta representa sólo el {pct_alta}%
      del corpus, pero concentra las quejas más graves: desabasto, muertes evitables,
      corrupción. Un F1 de <em>{d['f1_alta']:.3f}</em> sobre esta clase minoritaria
      indica que el modelo detecta la mayoría de casos críticos
      <strong>sin sacrificar demasiada precisión</strong>, lo cual es esencial
      para aplicaciones de vigilancia epidemiológica.
    </p>
  </div>
  <div class="card">
    ...
    <p style="font-size:.60rem;color:var(--muted);margin-top:8px;line-height:1.4">
      <strong style="color:var(--red)">Clase alta</strong> = queja sistémica grave
      (desabasto, espera extrema, muerte, corrupción).<br>
      <strong style="color:var(--blue)">Clase media</strong> = comentario sin queja grave
      o queja leve/informativa.
    </p>
    <p class="chart-desc" style="margin-top:8px">
      <strong>Origen:</strong> Comentarios extraídos de Reddit MX sobre influenza y
      atención hospitalaria pública. El etiquetado manual de {n_alta:,} quejas sistémicas
      constituyó la <em>gold standard</em> para entrenar el clasificador. La proporción
      {pct_alta}% / {round(100-pct_alta,1)}% refleja el desbalance real del problema:
      la mayoría de comentarios son informativos, pero la minoría crítica es la que
      importa detectar.
    </p>
  </div>
</div>
...
<div class="row row2" style="margin-bottom:14px">
  ...
  <div class="card accent-blue">
    ...
    <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
      ...
      <p class="chart-desc" style="margin-top:10px">
        <strong>¿Cómo funciona el soft voting?</strong> Cada clasificador emite una
        probabilidad P(alta|x). El ensamble promedia esas probabilidades ponderando
        más fuerte a LR y SVM (peso {w_lr}/{w_svm}) que a CNB (peso {w_cnb}),
        porque LR captura correlaciones BERT y SVM es más robusto ante léxico ruidoso.
        La predicción final es la clase con mayor probabilidad promedio ponderada.
        Este esquema reduce la varianza de cualquier clasificador individual y es
        especialmente útil en la <em>zona gris</em> donde los modelos discrepan.
      </p>
    </div>
  </div>
  ...
  <div class="card accent-red">
    ...
    <div style="display:flex;gap:18px;margin-bottom:14px;flex-wrap:wrap">
      ...
      <div style="flex:1;min-width:130px;background:rgba(233,69,96,.07);
        ...
        <div style="font-size:.76rem">
          {max(d['fn_diag'], key=d['fn_diag'].get) if d['fn_diag'] else 'texto muy corto'}</div>
    ...
    <div id="fn-bars"></div>
    <p class="chart-desc" style="margin-top:10px">
      <strong>Falsos Negativos (FN)</strong> son quejas sistémicas graves que el modelo
      clasifica erróneamente como "media" con alta confianza (≥80%). Cada FN representa
      una queja ignorada en una situación crítica de salud. Las barras muestran
      el nivel de confianza con la que el modelo se equivocó: cuanto más larga la barra,
      más "seguro" estaba el modelo al cometer el error. <strong>Son los errores
      de mayor costo</strong>, pues afectan directamente la detección de ineficiencia sistémica.
      Los <em>Falsos Positivos (FP)</em> son menos críticos: representan falsas alarmas
      que podrían revisarse manualmente.
    </p>
  </div>
</div>
...
<div class="row row3" style="margin-bottom:14px">
  <div class="card accent-purple">
    ...
    <div class="metric">
      ...
      <span class="sub">de {n_test:,} en test set ({round(tasa_des*100,1):.1f}% desacuerdo)</span>
      <span class="badge badge-purple">CNB ≠ LR ≠ SVM</span>
    </div>
    <p class="chart-desc">
      <strong>¿Qué es la zona gris?</strong> Son comentarios donde al menos uno de los
      tres clasificadores discrepa con los demás. Representan el <em>{round(tasa_des*100,1):.1f}%</em>
      del set de prueba y son los casos más ambiguos: contienen vocabulario intenso
      ("negligencia", "muerte", "desabasto") pero el contexto semántico no permite
      una clasificación inequívoca. Su existencia es evidencia directa de la
      <strong>ambigüedad inherente en el lenguaje de queja sistémica</strong>.
    </p>
  </div>
  <div class="card">
    ...
    <p class="chart-desc">
      <strong>Rescatados</strong> ({ens_resc}): casos donde los clasificadores
      discrepaban pero el ensamble acertó gracias al promedio ponderado de probabilidades.
      <strong>Zona gris real</strong> ({ens_fall}): casos donde ni siquiera el ensamble
      pudo resolver la ambigüedad. Estos son los comentarios con mayor carga semántica
      compleja: queja implícita, ironía o contexto indetectable por modelos basados en léxico.
    </p>
  </div>
  <div class="card">
    ...
    <p class="chart-desc">
      <strong>Tipo 2-1</strong>: Dos clasificadores predicen clase alta y uno predice media
      (o viceversa). El ensamble sigue la mayoría pero modera la confianza.
      <strong>Tipo 1-2</strong>: Un clasificador discrepa completamente de los otros dos.
      El ensamble suaviza con las probabilidades intermedias, resultando en predicciones
      de confianza media que señalan <em>ambigüedad lingüística genuina</em> en el texto.
    </p>
  </div>
</div>
...
<div class="row row2" style="margin-bottom:14px">
  <div class="card">
    ...
    <svg viewBox="0 0 480 260" xmlns="http://www.w3.org/2000/svg"
      ...
      <!-- Balanza inclinada correctamente: lado media (izq) más bajo porque n_media > n_alta -->
      <line x1="68" y1="117" x2="412" y2="83" stroke="#f0f0ff" stroke-width="4" stroke-linecap="round"/>
      <line x1="68"  y1="117" x2="68"  y2="168" stroke="#4cc9f0" stroke-width="2.5" stroke-dasharray="5,4"/>
      <line x1="412" y1="83"  x2="412" y2="134" stroke="#8888aa" stroke-width="2"   stroke-dasharray="5,4"/>
      <ellipse cx="68" cy="172" rx="55" ry="14" fill="#4cc9f0" opacity=".85"/>
      <text x="68" y="166" text-anchor="middle" font-size="10" font-weight="bold"
            fill="#042c53" font-family="sans-serif">Atención</text>
      <text x="68" y="179" text-anchor="middle" font-size="10" font-weight="bold"
            fill="#042c53" font-family="sans-serif">Normal</text>
      <text x="68" y="197" text-anchor="middle" font-size="9" fill="#4cc9f0"
            font-family="sans-serif">n={n_media:,}</text>
      <ellipse cx="412" cy="138" rx="58" ry="15" fill="#e94560" opacity=".90"/>
      <text x="412" y="132" text-anchor="middle" font-size="10" font-weight="bold"
            fill="#fff" font-family="sans-serif">Ineficiencia</text>
      <text x="412" y="145" text-anchor="middle" font-size="10" font-weight="bold"
            fill="#fff" font-family="sans-serif">Sistémica</text>
      <text x="412" y="160" text-anchor="middle" font-size="11" font-weight="bold"
            fill="#f4a261" font-family="sans-serif">DESABASTO</text>
      <text x="412" y="175" text-anchor="middle" font-size="11" font-weight="bold"
    ...
    <p class="chart-desc">
      <strong>¿Qué muestra la balanza?</strong> Es una metáfora visual del corpus:
      el lado izquierdo (azul, <em>Atención Normal</em>, n={n_media:,}) pesa más
      porque la mayoría de comentarios no contienen quejas graves. El lado derecho
      (rojo, <em>Ineficiencia Sistémica</em>, n={n_alta:,}) está más elevado —
      aunque con menor volumen, concentra los términos más intensos como
      <strong>DESABASTO</strong> y <strong>ESPERA extrema</strong>. La inclinación
      refleja el <strong>desbalance real del problema</strong>: la señal crítica existe
      pero es minoritaria, lo que justifica el uso de métricas ponderadas (F1-weighted)
      en lugar de accuracy simple.
    </p>
  </div>
  ...
  <div class="card">
    ...
    <p class="chart-desc" style="margin-top:10px">
      <strong>¿Cómo leer esta tabla?</strong> Cada celda cruza la etiqueta real (filas)
      con la predicción del modelo (columnas). <em>Verde (VP media)</em>: comentarios
      leves correctamente ignorados. <em>Azul (VP alta)</em>: quejas sistémicas detectadas
      con éxito — el objetivo principal. <em>Rojo (FN)</em>: quejas graves clasificadas
      como leves; son los errores más costosos porque implican que una denuncia de
      desabasto o muerte evitable pasa desapercibida. <em>Ámbar (FP)</em>: falsas alarmas,
      manejables con revisión manual. <strong>La asimetría entre FN y FP</strong>
      refleja la estrategia del modelo: preferir detectar más quejas reales aunque
      alguna sea falsa alarma.
    </p>
  </div>
</div>
...
<div class="card" style="margin-bottom:14px">
  ...
  <p class="chart-desc" style="margin-top:10px">
    <strong>¿Qué muestra esta gráfica?</strong> Cada barra representa una feature
    del modelo de Regresión Logística (sub-estimador del ensamble) y su coeficiente
    absoluto para predecir <em>clase alta</em> (queja sistémica grave). La longitud
    de la barra indica el <strong>peso predictivo</strong>: cuanto más larga, más
    decisiva es esa feature para la clasificación. El símbolo <strong>▲</strong>
    significa que la feature empuja hacia clase alta; <strong>▼</strong> hacia clase media.
    El color indica el origen: <em>azul</em> = embedding BERT (semántica profunda),
    <em>verde</em> = feature de enriquecimiento manual (léxico de salud), 
    <em>rojo</em> = término TF-IDF puro (frecuencia léxica en el corpus).
  </p>
</div>
...
<div class="card" style="margin-bottom:14px">
  ...
  <p class="chart-desc" style="margin-top:8px">
    <strong>¿Cómo leer el heatmap?</strong> Cada fila es una palabra clave del vocabulario
    predictivo de clase alta. Las columnas representan rangos del puntaje de negatividad
    semántica asignado por BERT (de "muy positivo" a "muy negativo"). El color de cada
    celda indica qué tan frecuentemente esa palabra aparece en comentarios con ese nivel de
    negatividad: <em>frío/oscuro</em> = baja co-ocurrencia, <em>rojo intenso</em> = alta
    co-ocurrencia. La <strong>línea discontinua roja</strong> separa los textos neutros/positivos
    de los genuinamente negativos (prob_neg ≥ 0.6). Un patrón donde las palabras clave
    se concentran a la derecha de esa línea confirma que el léxico de queja sistémica
    <strong>correlaciona con negatividad semántica profunda</strong>, no solo con frecuencia léxica.
  </p>
</div>
...
<div class="row row3" style="margin-bottom:14px">
  <div class="card">
    ...
    <p class="chart-desc" style="margin-top:8px">
      <strong>¿Por qué el ensamble gana?</strong> Cada barra muestra el F1-weighted
      obtenido por un clasificador distinto sobre el mismo corpus. El
      <em>VotingEnsemble</em> combina las fortalezas de los tres: CNB es rápido
      y maneja bien el desbalance de clases, LR aprovecha embeddings BERT, y
      SVM maximiza el margen de separación con léxico ruidoso. Ninguno es
      superior en todos los escenarios, pero su combinación ponderada
      <strong>supera consistentemente a cualquier clasificador individual</strong>,
      validando la arquitectura de ensamble como estrategia central del pipeline.
    </p>
  </div>
  <div class="card">
    ...
    <p class="chart-desc" style="margin-top:8px">
      <strong>¿Qué es la validación cruzada 10-fold?</strong> El corpus se divide en
      10 partes iguales. El modelo se entrena 10 veces, cada vez usando 9 partes para
      entrenar y 1 para evaluar. Cada barra representa el F1 en esa ronda. Si las barras
      tienen alturas similares (σ = {d['f1_std']:.3f}), el modelo es
      <em>estable</em>: no depende del subconjunto de datos que vea. Un modelo inestable
      tendría barras muy dispares, señal de que aprende patrones superficiales o aleatorios
      en lugar de patrones genuinos de queja sistémica.
    </p>
  </div>
  <!-- Overfitting check -->
  <div class="card">
    ...
    <p class="chart-desc" style="margin-top:10px">
      <strong>¿Qué es el overfitting?</strong> El gap (Δ) es la diferencia entre
      el F1 en entrenamiento y en test. Un gap de <em>{gap:+.4f}</em> significa que
      el modelo generaliza bien: lo que aprende del corpus de entrenamiento
      <strong>se transfiere sin degradarse</strong> a datos nuevos. Si el gap fuera
      mayor al 10%, el modelo estaría "memorizando" el entrenamiento en lugar de aprender
      patrones reales. La <em>curva de aprendizaje</em> muestra cómo evolucionan ambas
      métricas a medida que se añaden más ejemplos de entrenamiento: la convergencia
      entre las dos líneas confirma que el modelo aprovecha bien los datos disponibles.
    </p>
  </div>
</div>
...
<div class="row row2" style="margin-bottom:14px">
  <div class="card">
    <div class="card-title">Distribución de confianza — P(alta|x) en predicciones test</div>
    <div id="dist-conf-bars"></div>
    <p style="font-size:.63rem;color:var(--muted);margin-top:8px">
      Binning sobre P(alta|x) ≥ 0.5 (predicciones "alta"). Alta concentración
      en [0.9–1.0] indica que el ensamble discrimina con alta certeza.
    </p>
    <p class="chart-desc" style="margin-top:8px">
      <strong>¿Qué muestran las barras de confianza?</strong> Cada barra agrupa
      comentarios clasificados como "alta" según el nivel de certeza del ensamble:
      P(alta|x) es la probabilidad que asigna el modelo. Barras altas en el rango
      <em>[0.9–1.0]</em> significan que cuando el modelo predice una queja sistémica,
      lo hace con gran seguridad — señal de un clasificador bien calibrado. Si la
      mayor parte de las predicciones estuviera en [0.5–0.6], indicaría que el modelo
      "adivina" más que decide. <strong>El Brier score</strong> (calibración = {d['brier_cal']:.5f})
      cuantifica globalmente qué tan bien alineadas están las probabilidades con la realidad.
    </p>
  </div>
  <div class="card">
    ...
    <p class="chart-desc" style="margin-top:8px">
      <strong>¿Qué son las dimensiones narrativas?</strong> Los términos predictivos
      se agrupan en categorías temáticas que reflejan distintos tipos de queja sistémica:
      <em>falta de insumos</em> (medicamentos, equipos), <em>tiempo de espera</em> extremo,
      <em>consecuencia clínica</em> (muertes, agravamiento), <em>corrupción/negligencia</em>,
      <em>sistema roto</em> (burocracia, traslados) y <em>pago privado</em> forzado.
      La longitud de cada barra indica qué <strong>fracción del vocabulario predictivo</strong>
      pertenece a esa dimensión, revelando cuál tipo de queja domina el discurso de
      ineficiencia sistémica en Reddit MX.
    </p>
  </div>
</div>
...
<div class="row row2" style="margin-bottom:14px">
  <div class="card">
    ...
    <p class="chart-desc" style="margin-top:8px">
      <strong>¿Qué mide la correlación de Spearman?</strong> A diferencia de Pearson,
      Spearman no asume distribución normal: mide si a mayor valor de una feature numérica
      (longitud del texto, conteo de términos de queja, puntaje BERT), mayor es la
      probabilidad de que el comentario sea clase alta. <em>ρ &gt; 0</em> (barra verde)
      indica asociación positiva con ineficiencia sistémica; <em>ρ &lt; 0</em> (barra roja)
      asociación inversa. El asterisco <strong>*</strong> marca correlaciones
      estadísticamente significativas (p &lt; 0.05), descartando que sean producto del azar.
    </p>
  </div>
  <div class="card">
    ...
    <p class="chart-desc" style="margin-top:8px">
      <strong>¿Qué miden χ² y Odds Ratio?</strong> El test de chi-cuadrado evalúa si
      la presencia de una categoría de queja (desabasto, espera, negligencia…) es
      <em>independiente</em> de la clase o si hay asociación estadística real. La longitud
      de la barra (χ²) indica la fuerza de la evidencia. El <strong>Odds Ratio (OR)</strong>
      mostrado a la derecha cuantifica la magnitud: OR = 2 significa que un comentario
      de esa categoría tiene el <em>doble de probabilidad</em> de ser clase alta que uno
      sin esa categoría. OR &gt; 1.5 se considera asociación fuerte; OR &lt; 0.67 indica
      que esa categoría es <strong>protectora</strong> contra la clasificación como queja grave.
    </p>
  </div>
</div>{pngs_section}
...
(function(){{
  ...
  // Escala: el mínimo ocupa 10% de altura, el máximo 100%
  const rangeV=maxV-minV || 0.01;
  FOLDS.forEach((v,i)=>{{
    const pct=Math.max(10,Math.round(10+(v-minV)/rangeV*90));
    const isMax=v===maxV;
...
(function(){{
  ...
  if(!CORS.length){{
    cont.innerHTML='<p style="color:#8888aa;font-size:.75rem;padding:8px 0">'+
      'No disponible — analizar_v4.py debe ejecutarse con el argumento '+
      '<code style="background:rgba(255,255,255,.07);padding:1px 5px;border-radius:3px">dataset_etiquetado*.json</code> '+
      'para calcular correlaciones Spearman.</p>';
    return;
  }}
...
(function(){{
  ...
  if(!CHI2.length){{
    cont.innerHTML='<p style="color:#8888aa;font-size:.75rem;padding:8px 0">'+
      'No disponible — analizar_v4.py debe ejecutarse con el argumento '+
      '<code style="background:rgba(255,255,255,.07);padding:1px 5px;border-radius:3px">dataset_etiquetado*.json</code> '+
      'para calcular chi² y Odds Ratio por categoría.</p>';
    return;
  }}
...
def main():
    ...
    disc = d["disc_resumen"]
    tiene_cors  = bool(d["correlaciones"])
    tiene_chi2  = bool(d["chi2"])
    tiene_feats = bool(d["tabla_hibrida"] or d["top_alta"])

    print(f"\n{'='*64}")
    print(f"  ✅ Infografía v4 generada:")
    print(f"     {salida.name}")
    print(f"\n  Corpus leído del reporte:")
    print(f"     Total  : {d['n_total']:,}  |  Alta: {d['n_alta']:,}  |  Media: {d['n_media']:,}")
    print(f"     F1 CV  : {d['f1']:.4f} ± {d['f1_ic']:.4f}  |  AUC: {d['auc']:.4f} ± {d['auc_ic']:.4f}")
    print(f"\n  Secciones incluidas:")
    ...
    print(f"     ③ Zona gris / discrepancias: {disc.get('n_discrepancias',0)} casos "
          f"({disc.get('tasa_desacuerdo',0)*100:.1f}% del test set)")
    print(f"     ④ Balanza conceptual (corregida) + Matriz de confusión")
    print(f"     ⑤ Top features {'✓' if tiene_feats else '⚠ sin datos'} + Heatmap co-ocurrencia")
    print(f"     ⑥ Comparación modelos + Estabilidad 10-fold + Overfitting")
    print(f"     ⑦ Distribución confianza + Dimensiones narrativas")
    print(f"     ⑧ Correlaciones Spearman {'✓' if tiene_cors else '⚠ sin datos — ejecutar con dataset JSON'}")
    print(f"         Chi² por categoría   {'✓' if tiene_chi2 else '⚠ sin datos — ejecutar con dataset JSON'}")
